...
def __verifierZKP(c_client, Zx_client, _g0, _Y, token):
    c_server = []
    for i in range(len(c_client)):
        _Ts = ((pow(((int(c_client[i],16))%10**2), _g0) % _g0)*_Y + pow(Zx_client[i], _g0)%_g0) % _g0
...
def loginClient(conn_client):
    ...
    if client != None:
        ...
        _g0 = client["g"]
        _Y = client["y"]
        _list_c_server = __verifierZKP(_list_c_client, _list_Zx_client, _g0, _Y, token)
